...
async function loadConfig() {
  try {
    ...
    if (!response.ok) {
      console.warn('設定の読み込みに失敗しました:', response.status);
      return false;
    }
...
document.addEventListener('DOMContentLoaded', async function() {
  // 設定を読み込む
  await loadConfig();
  console.log('設定読み込み完了、現在の設定:', config);
  ...
  screenshotBtn.addEventListener('click', async (event) => {
    ...
    try {
      // リクエスト先のURLをログに出力
      const requestUrl = `${config.API_BASE_URL}/api/screenshot`;
  ...
  youtubeBtn.addEventListener('click', async (event) => {
    ...
    try {
      // リクエスト先のURLをログに出力
      const requestUrl = `${config.API_BASE_URL}/api/youtube-screenshot`;
      console.log('APIリクエスト先:', requestUrl);
  ...
  shortsBtn.addEventListener('click', async (event) => {
    ...
    try {
      // リクエスト先のURLをログに出力
      const requestUrl = `${config.API_BASE_URL}/api/shorts-screenshot`;
      console.log('APIリクエスト先:', requestUrl);
      console.log('リクエストボディ:', { video_id: videoId });
  ...
  batchBtn.addEventListener('click', async (event) => {
    ...
    try {
      ...
      // リクエスト先のURLをログに出力
      const requestUrl = `${config.API_BASE_URL}/api/screenshot/batch`;
  ...
  downloadAllBtn.addEventListener('click', async () => {
    ...
    try {
      ...
      const zipUrl = `${config.API_BASE_URL}/api/screenshot/download-zip/${batchDirName}`;
